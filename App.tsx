...
const FieldWorkerTool: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch (e) { console.error(e); alert("Failed to log project."); } finally { setIsSubmitting(false); }
  };
...
export default function App() {
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (passwordInput === import.meta.env.VITE_ADMIN_PASSWORD || passwordInput === 'SN') {
          ...
          <div className="hidden md:flex items-center gap-10">
            ...
            {isAdminUnlocked && (
              <>
                ...
                <Button onClick={() => { setIsAdminUnlocked(false); setView(NavView.HOME); }} variant="outline" className="!py-2.5 !px-5 !text-sm border-rose-200 text-rose-600 hover:bg-rose-50 hover:border-rose-300"><LogOut className="w-4 h-4" /> Logout</Button>
              </>
            )}
            ...
            <div className="flex flex-col p-4 bg-white/80">
              ...
              {isAdminUnlocked && (
                <>
                  ...
                  <button
                    onClick={() => { setIsAdminUnlocked(false); setView(NavView.HOME); setIsMenuOpen(false); }}
