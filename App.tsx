...
const CLOUDINARY_URL = "https://api.cloudinary.com/v1_1/sunil-fabrications/image/upload";
const CLOUDINARY_UPLOAD_PRESET = "unsigned_preset"; // User should set this up in Cloudinary dashboard
...
const FieldWorkerTool: React.FC = () => {
  ...
  const addMeasurement = () => {
    setMeasurements([...measurements, { type: 'Window', style: 'Standard', label: '', width: '', height: '', quantity: 1 }]);
  };
  ...
  const updateMeasurement = (index: number, field: string, value: string | number) => {
    ...
    if (field === 'type') {
      const defaultStyle = STYLE_OPTIONS[value as keyof typeof STYLE_OPTIONS]?.[0] || 'Standard';
      newM[index].style = defaultStyle;
    }

    setMeasurements(newM);
  };

  const addVentilatorItem = () => {
    setVentilators([...ventilators, { label: '', quantity: 1, unitPrice: 0 }]);
  };

  const removeVentilatorItem = (index: number) => {
    setVentilators(ventilators.filter((_, i) => i !== index));
  };
  ...
  const getItemRate = (m: typeof measurements[0]) => {
    ...
    switch (m.type) {
      case 'Window': return rates.window;
      case 'Door': return rates.door;
      case 'Mess': return rates.mess;
      case 'Elevation': return rates.elevation;
      case 'Partition': return rates.partition;
      case 'Glass': return rates.glass;
      default: return 0;
    }
  };
  ...
  const itemsCost = useMemo(() => {
    return measurements.reduce((acc, m) => {
      ...
      const rate = getItemRate(m);
      return acc + (w * h * q * rate);
    }, 0);
  }, [measurements, rates]);

  const ventilatorsCost = useMemo(() => {
    return ventilators.reduce((acc, v) => {
      const q = parseInt(v.quantity.toString()) || 0;
      const p = parseFloat(v.unitPrice.toString()) || 0;
      return acc + (q * p);
    }, 0);
  }, [ventilators]);

  const totalCost = useMemo(() => {
    return itemsCost + ventilatorsCost;
  }, [itemsCost, ventilatorsCost]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!clientName || !clientPhone) return alert("Client details are essential!");
    if (measurements.some(m => !m.type || !m.width || !m.height)) return alert("Please fill all measurement details.");
    setIsSubmitting(true);

    try {
      // Upload images to Cloudinary
      const uploadedPhotoUrls = await Promise.all(imagePreviews.map(async (img) => {
        if (img.startsWith('http')) return img; // Already uploaded or external
      ...
      const compatibleMeasurements: Measurement[] = measurements.map(m => ({
        label: `${m.type} (${m.style})${m.label ? ` - ${m.label}` : ''}`,
        width: m.width,
        height: m.height,
        quantity: m.quantity
      }));
      ...
      const newProject = {
        ...
        ventilators: ventilators.length > 0 ? ventilators : [],
      ...
      alert("Measurements & Ventilators Logged Successfully!");

      // Reset form
      setClientName(''); setClientPhone(''); setLocation(''); setLatLng(null);
      setMeasurements([{ type: 'Window', style: 'Standard', label: '', width: '', height: '', quantity: 1 }]);
      setVentilators([]);
      setImagePreviews([]);
      setManualEstimatedCost('');
    } catch (e) {
      console.error(e);
      alert("Error logging project. Check your connection and Cloudinary settings.");
    } finally {
  ...
  const formatWhatsAppText = () => {
    ...
    const ventText = ventilators.length > 0
      ? `%0A*Ventilators:*%0A${ventilators.map(v => `• ${v.label}: ${v.quantity} Units @ Rs. ${v.unitPrice}/ea`).join('%0A')}`
      : '';

    const itemText = measurements.map(m => {
      const typeLabel = `${m.type} (${m.style})`;
      const locLabel = m.label ? ` - ${m.label}` : '';
      return `• ${typeLabel}${locLabel}: ${m.width} x ${m.height} (Qty: ${m.quantity})`;
    }).join('%0A');

    return `*SUNIL FABRICATIONS VISIT LOG*%0A%0A*Client:* ${clientName}%0A*Phone:* ${clientPhone}%0A*Location:* ${location}${mapText}%0A%0A*Measurements (Units: ${selectedUnit}):*%0A${itemText}${ventText}%0A%0A*Calculated Total:* Rs. ${totalCost.toLocaleString()}%0A*Field Quote:* Rs. ${manualEstimatedCost || 'N/A'}`;
  };
  ...
  return (
    <div id="field-worker-tool" className="max-w-5xl mx-auto py-20 px-6">
      <Reveal>
          ...
          <form onSubmit={handleSubmit} className="p-8 md:p-12 space-y-10">
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="space-y-3 md:col-span-1">
                ...
                <p className="text-[10px] text-slate-400 italic">Click once you are at exact site location.</p>
              </div>
              <div className="space-y-3 md:col-span-1">
            ...
            {/* UPDATED ITEM DIMENSIONS SECTION WITH TYPES */}
                ...
                <table className="w-full border-collapse bg-white">
                  <thead className="bg-slate-50">
                    <tr className="text-left">
                      <th className="py-4 px-4 font-black text-slate-400 text-[10px] uppercase tracking-widest">Type</th>
                      <th className="py-4 px-4 font-black text-slate-400 text-[10px] uppercase tracking-widest">Style</th>
                    ...
                    {measurements.map((m, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                        <td className="py-3 px-4">
                          <select
                            value={m.type}
                            onChange={e => updateMeasurement(idx, 'type', e.target.value)}
                            className="w-full bg-white border-2 border-slate-100 px-2 py-2 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-xs font-bold transition-all text-blue-600"
                          >
                        ...
                        <td className="py-3 px-4">
                          <select
                            value={m.style}
                            onChange={e => updateMeasurement(idx, 'style', e.target.value)}
                            className="w-full bg-white border-2 border-slate-100 px-2 py-2 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-xs font-bold transition-all text-slate-600"
                          >
                            {(STYLE_OPTIONS[m.type as keyof typeof STYLE_OPTIONS] || ['Standard']).map(s => <option key={s} value={s}>{s}</option>)}
                          </select>
                        </td>
                        <td className="py-3 px-6"><input value={m.label} onChange={e => updateMeasurement(idx, 'label', e.target.value)} className="w-full bg-white border-2 border-slate-100 px-4 py-2.5 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm font-medium transition-all" placeholder="e.g. Kitchen" /></td>
                        <td className="py-3 px-4"><input required type="text" value={m.width} onChange={e => handleNumericInput(idx, 'width', e.target.value)} className={`w-20 bg-gradient-to-br from-blue-50/80 to-blue-100/50 border-2 ${!m.width ? 'border-rose-200' : 'border-blue-200'} px-3 py-2.5 rounded-xl text-center text-sm font-black text-blue-700 focus:bg-white focus:border-blue-500 outline-none transition-all shadow-inner`} placeholder="0.0" /></td>
                        <td className="py-3 px-4"><input required type="text" value={m.height} onChange={e => handleNumericInput(idx, 'height', e.target.value)} className={`w-20 bg-gradient-to-br from-indigo-50/80 to-indigo-100/50 border-2 ${!m.height ? 'border-rose-200' : 'border-indigo-200'} px-3 py-2.5 rounded-xl text-center text-sm font-black text-indigo-700 focus:bg-white focus:border-indigo-500 outline-none transition-all shadow-inner`} placeholder="0.0" /></td>
            ...
            <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-bold text-slate-900 tracking-tight">Smart Pricing Logic</h3>
              </div>
              <p className="text-sm text-slate-500 mb-6">Set base prices per unit area. Track styles (2/3 Track) override category rates automatically.</p>

              <div className="space-y-6">
                {/* Category Group */}
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Window</label>
                    <input type="number" value={rates.window} onChange={e => setRates({ ...rates, window: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold text-blue-700 outline-none focus:border-blue-500 shadow-sm transition-all" placeholder="0" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Door</label>
                    <input type="number" value={rates.door} onChange={e => setRates({ ...rates, door: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold text-blue-700 outline-none focus:border-blue-500 shadow-sm transition-all" placeholder="0" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Mess</label>
                    <input type="number" value={rates.mess} onChange={e => setRates({ ...rates, mess: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold text-blue-700 outline-none focus:border-blue-500 shadow-sm transition-all" placeholder="0" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Elev.</label>
                    <input type="number" value={rates.elevation} onChange={e => setRates({ ...rates, elevation: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold text-blue-700 outline-none focus:border-blue-500 shadow-sm transition-all" placeholder="0" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Part.</label>
                    <input type="number" value={rates.partition} onChange={e => setRates({ ...rates, partition: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold text-blue-700 outline-none focus:border-blue-500 shadow-sm transition-all" placeholder="0" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Glass</label>
                    <input type="number" value={rates.glass} onChange={e => setRates({ ...rates, glass: parseFloat(e.target.value) || 0 })} className="w-full px-3 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold text-blue-700 outline-none focus:border-blue-500 shadow-sm transition-all" placeholder="0" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-2 gap-6">
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-indigo-400 uppercase tracking-widest ml-1">2 Track Override</label>
                    <input type="number" value={rates.twoTrack} onChange={e => setRates({ ...rates, twoTrack: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-2.5 bg-indigo-50 border-2 border-indigo-100 rounded-xl text-sm font-bold text-indigo-700 outline-none focus:border-indigo-500 shadow-sm transition-all" placeholder="0" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-indigo-400 uppercase tracking-widest ml-1">3 Track Override</label>
                    <input type="number" value={rates.threeTrack} onChange={e => setRates({ ...rates, threeTrack: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-2.5 bg-indigo-50 border-2 border-indigo-100 rounded-xl text-sm font-bold text-indigo-700 outline-none focus:border-indigo-500 shadow-sm transition-all" placeholder="0" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 space-y-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-indigo-100 text-indigo-600 rounded-2xl flex items-center justify-center">
                    <VentilatorIcon className="w-6 h-6" />
                  </div>
                  <h3 className="text-2xl font-bold text-slate-900 tracking-tight">Ventilators</h3>
                </div>
                <button type="button" onClick={addVentilatorItem} className="text-indigo-600 font-bold hover:text-indigo-800 flex items-center gap-2 px-4 py-2 bg-indigo-50 rounded-xl transition-all text-sm"><Plus className="w-4 h-4" /> Add Item</button>
              </div>

              {ventilators.length === 0 && (
                <div className="text-center py-8 text-slate-400 font-medium italic border-2 border-dashed border-slate-200 rounded-2xl">
                  No ventilators added. Click "Add Item" to specify types.
                </div>
              )}

              <div className="space-y-4">
                {ventilators.map((vent, idx) => (
                  <div key={idx} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-12 gap-4 items-center relative group">
                    <div className="md:col-span-5 space-y-1">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Label</label>
                      <input
                        type="text"
                        value={vent.label}
                        onChange={e => updateVentilatorItem(idx, 'label', e.target.value)}
                        className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:border-indigo-500 outline-none text-sm font-bold text-slate-700 transition-all"
                    ...
                    <div className="md:col-span-3 space-y-1">
                      ...
                      <input
                        type="number"
                        value={vent.quantity}
                        onChange={e => updateVentilatorItem(idx, 'quantity', parseInt(e.target.value) || 0)}
                        className="w-full px-4 py-2.5 bg-indigo-50 border border-indigo-100 rounded-xl focus:border-indigo-500 outline-none text-center text-sm font-black text-indigo-700 transition-all"
                        placeholder="0"
                      />
                    </div>
                    <div className="md:col-span-3 space-y-1">
                      ...
                      <input
                        ...
                        onChange={e => updateVentilatorItem(idx, 'unitPrice', parseFloat(e.target.value) || 0)}
                        className="w-full px-4 py-2.5 bg-indigo-50 border border-indigo-100 rounded-xl focus:border-indigo-500 outline-none text-center text-sm font-black text-indigo-700 transition-all"
                        placeholder="0"
                      />
                    </div>
                    <div className="md:col-span-1 flex justify-center pt-5">
                      <button type="button" onClick={() => removeVentilatorItem(idx)} className="p-2.5 text-rose-400 hover:bg-rose-50 hover:text-rose-600 rounded-xl transition-colors"><Trash2 className="w-5 h-5" /></button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {ventilators.length > 0 && (
                <div className="pt-4 border-t border-indigo-100 flex justify-end items-center gap-4">
                  <span className="text-sm font-bold text-slate-500">Subtotal:</span>
                  <span className="text-xl font-black text-indigo-600">₹ {ventilatorsCost.toLocaleString()}</span>
                </div>
              )}
            </div>

            {/* COST SUMMARY SECTION */}
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-8 rounded-[2.5rem] shadow-xl border border-slate-700 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <h3 className="text-2xl font-bold tracking-tight">Cost Summary</h3>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  <div className="bg-white/10 p-6 rounded-2xl border border-white/20 backdrop-blur-sm">
                    <p className="text-slate-400 font-medium text-sm mb-1">Glass & Items</p>
                    <p className="text-3xl font-black">₹ {itemsCost.toLocaleString()}</p>
                  </div>
                  <div className="bg-white/10 p-6 rounded-2xl border border-white/20 backdrop-blur-sm">
                    <p className="text-slate-400 font-medium text-sm mb-1">Ventilators</p>
                    <p className="text-3xl font-black">₹ {ventilatorsCost.toLocaleString()}</p>
                  </div>
                </div>

                <div className="border-t border-white/20 pt-6 space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-lg font-bold text-blue-400 uppercase tracking-widest">Total Estimate</span>
                    <span className="text-4xl font-extrabold text-blue-400">₹ {totalCost.toLocaleString()}</span>
                  </div>
                </div>
              </div>

              <div className="mt-10 pt-8 border-t border-white/20 space-y-4 relative z-10">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">Manually Quoted Cost (Field)</label>
                <div className="relative bg-white/10 rounded-2xl border border-white/30 focus-within:border-blue-400 transition-colors">
                  <span className="absolute left-5 top-1/2 -translate-y-1/2 font-black text-slate-400 text-xl">₹</span>
                  <input
                    type="text"
                    value={manualEstimatedCost}
                    onChange={e => setManualEstimatedCost(e.target.value.replace(/[^0-9]/g, ''))}
                    className="w-full pl-12 pr-6 py-5 bg-transparent outline-none font-black text-xl text-white placeholder-slate-600 transition-all"
                    placeholder="Enter final price given to client"
                  />
                </div>
                <p className="text-xs text-slate-500 italic text-center">Use this to override calculation or match quote promised on site.</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
                <label className="cursor-pointer bg-slate-50 hover:bg-blue-50 border-2 border-dashed border-slate-200 hover:border-blue-400 aspect-square rounded-3xl flex flex-col items-center justify-center transition-all group p-4 text-center shadow-sm hover:shadow-md">
...
const PortfolioManager: React.FC<{
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(e);
      alert("Error uploading to Cloudinary or saving to Firebase. Ensure you have set up an unsigned upload preset named 'unsigned_preset' in Cloudinary.");
    } finally {
            ...
            {filteredProjects.map((p) => (
              <div key={p.id} className="bg-white rounded-[2rem] shadow-xl border border-slate-100 hover:shadow-2xl transition-all relative group overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                  {/* Header Grid */}
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start border-b border-slate-50 pb-4">
                    <div className="md:col-span-2">
                      <h3 className="text-2xl font-black text-slate-900 leading-none">{p.clientName}</h3>
                    ...
                    <div className="flex justify-end">
                      <select value={p.status} onChange={e => updateProjectStatus(p.id, e.target.value as ProjectStatus)} className={`text-[10px] font-black uppercase tracking-widest border rounded-lg px-3 py-1 outline-none transition-colors cursor-pointer ${p.status === ProjectStatus.PENDING ? 'bg-slate-100 text-slate-500 border-slate-200' :
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {(p as any).rawMeasurements ? (
                          (p as any).rawMeasurements.slice(0, 3).map((m: any, i: number) => (
                            <div key={i} className={`inline-flex items-center px-2.5 py-1.5 rounded-lg text-[10px] font-bold border ${m.type === 'Window' || m.type === 'Door' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                              m.type === 'Mess' || m.type === 'Glass' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' :
                                'bg-emerald-50 text-emerald-700 border-emerald-200'
                              }`}>
                              <span>{m.type.substring(0, 3)}</span>
                          ...
                          <span className="text-sm font-bold text-slate-700">Click to view details</span>
                        )}
                      </div>
                      {(p.ventilators && Array.isArray(p.ventilators) && p.ventilators.length > 0) && (
                        <div className="flex flex-wrap gap-2 mt-2 pt-2 border-t border-slate-100">
                          {p.ventilators.slice(0, 2).map((v, i) => (
                            <div key={i} className="inline-flex items-center px-2.5 py-1.5 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-lg text-[10px] font-bold">
                              <VentilatorIcon className="w-3 h-3 mr-1" />
                              {v.label || 'Vent'} ({v.quantity})
                            </div>
                          ))}
                          {p.ventilators.length > 2 && (
                            <div className="inline-flex items-center px-2 py-1.5 bg-slate-100 text-slate-500 rounded-lg text-[10px] font-bold">
                              +{p.ventilators.length - 2}
                            </div>
                          )}
                        </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-xl font-black text-slate-900">₹{p.totalCost?.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between items-center border-t border-slate-200 pt-2">
                          <span className="text-xs text-slate-500 font-medium">Estimated</span>
                          <span className="text-xl font-black text-blue-600">₹{p.estimatedCost?.toLocaleString() || '---'}</span>
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  return (
    <div className="min-h-screen relative font-sans text-slate-900 overflow-x-hidden">
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4">
        <div className="container mx-auto max-w-7xl glass-card rounded-[2rem] shadow-xl border border-white/50 px-6 md:px-8 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-10">
            ...
            {isAdminUnlocked && (<><button onClick={() => setView(NavView.HOME)} className={`font-bold text-sm tracking-tight transition-all hover:text-blue-600 ${view === NavView.HOME ? 'text-blue-600' : 'text-slate-500'}`}>HOME</button><button onClick={() => setView(NavView.GALLERY)} className={`font-bold text-sm tracking-tight transition-all hover:text-blue-600 ${view === NavView.GALLERY ? 'text-blue-600' : 'text-slate-500'}`}>PORTFOLIO</button><button onClick={() => setView(NavView.WORKFLOW)} className={`font-bold text-sm tracking-tight transition-all hover:text-blue-600 ${view === NavView.WORKFLOW ? 'text-blue-600' : 'text-slate-500'}`}>OUR JOURNEY</button><button onClick={() => setView(NavView.FIELD_TOOL)} className={`font-bold text-sm tracking-tight transition-all hover:text-blue-600 ${view === NavView.FIELD_TOOL ? 'text-blue-600' : 'text-slate-500'}`}>FIELD TOOL</button><Button variant="secondary" onClick={() => setView(NavView.ADMIN)} className="!py-2.5 !px-5 !text-sm"><LayoutDashboard className="w-4 h-4" /> ADMIN</Button><Button onClick={() => { setIsAdminUnlocked(false); setView(NavView.HOME); }} variant="outline" className="!py-2.5 !px-5 !text-sm border-rose-200 text-rose-600 hover:bg-rose-50 hover:border-rose-300"><LogOut className="w-4 h-4" /> Logout</Button></>)}
      ...
      <main className="pt-24 pb-20">
        ...
        {view === NavView.ADMIN && <AdminDashboard projects={projects} galleryItems={galleryItems} feedback={feedback} onLogout={() => { setIsAdminUnlocked(false); setView(NavView.HOME); }} />}
      </main>
